...
function App() {
  ...
  useEffect(() => {
    ...
    if (isSignedIn && account && currentPath !== "/payment/success") {
      dispatch(userCart(account.id));
    }
  }, [dispatch, isSignedIn, account, currentPath]);
  ...
  useEffect(() => {
    ...
    if (changedCart && isSignedIn && account && currentPath !== "/payment/success") {
      const cartdata = cart.map((item) => {
        return { productid: item._id, quantity: item.quantity };
      });
      dispatch(
  ...
  }, [dispatch, changedCart, isSignedIn, account, cart, currentPath]);
