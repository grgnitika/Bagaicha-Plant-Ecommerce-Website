...
// Converts "Plant Pots" => "plant-pots"
const slugify = (text) =>
  text
    .toLowerCase()
    .trim()
...
function CategoriesForm() {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      delete jsonPayload.thumbnail;
      delete jsonPayload.breadcrumbbanner;

      if (data.thumbnail instanceof File) {
        formData.append("thumbnail", data.thumbnail);
      }
      if (data.breadcrumbbanner instanceof File) {
        formData.append("breadcrumbbanner", data.breadcrumbbanner);
      }

      formData.append("inputdata", JSON.stringify(jsonPayload));
      ...
      <FormControl>
        <input
          ...
          onChange={(e) => {
            ...
            field.onChange(name);
            if (!isEditForm) {
              form.setValue("categoriesid", slugify(name));
            }
          }}
        />
      </FormControl>
